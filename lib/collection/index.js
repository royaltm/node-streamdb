...
const { assertConstantsDefined, isPlainObject } = require('../util');

const { iter, mixin } = require('../iter');

const { createCollectionItemKlass, Item } = require('./item');
const { COLLECTION_NAME_MATCH, createSchema
      , isSchemaCompositeIndex, addSchemaDescriptor, addSchemaCompositeIndex } = require('./schema');
...
class Collection {
  constructor(db, name) {
    ...
    this[itemKlass$] = createCollectionItemKlass(this, Model);
  ...
  [applySchema$](schemaCfg, validators) {
    ...
    if (schemaCfg) {
      ...
      for(let property of indexes) {
        addSchemaCompositeIndex.call(this, property, schemaCfg);
      }
    }
  ...
  update(id, value) {
    ...
    for(var name in value) {
      if (hasOwnProperty.call(value, name)) {
        if (name.length === 0) throw new TypeError("update: property name must not be empty");
  ...
  _addElement(id, property, value) {
    ...
    if (descr !== undefined) value = descr[validateElement$](value, '+');
  ...
  _replace(id, property, value) {
    ...
    if (property.length === 0) {
      if (value !== undefined) { // yes we can delete an item regardless of its schema
      ...
      let descr = this[schema$][property];
      if (descr !== undefined) value = descr[validate$](value);
    }
