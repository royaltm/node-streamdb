...
const { COLLECTION_NAME_MATCH
      ...
      , validateDeepPropertyCrossReference } = require('./schema');
const { exportSchemaItemNoManyReferences } = require('./schema/utils');
...
const collectionProxyHandler = {
  get(collection, name) {
    ...
    if (undefined === value && 'string' === typeof name) {
      ...
      return () => `\u001b[90mCollection(${collection._name})\u001b[39m \u001b[90m(${collection[items$].size})\u001b[39m`;
    }
    return value;
  },
  ...
  */
  ...
  _addElement(id, property, value) {
    ...
    if (descr === undefined) {
      ...
      validateDeepPropertyCrossReference(property, schema);
    }
    else {
...
Collection.prototype[export$] = function* (select) {
  const collname = this._name
      , schema = this[schema$];

  if (select !== hasManyReferencesOnly$) {
    yield [collname, '!', null, null, null];

    for(let proxy of this[items$].values()) {
      let item = proxy[this$];
      try {
        yield [collname, '=', new Ident(item[id$]), '',
                exportSchemaItemNoManyReferences(create(null), schema, item)];
      } catch(e) {
  ...
  if (select !== skipHasManyReferences$) {
    for(const {name, readPropertySymbol} of schema[hasManyDescriptorsIterate$]()) {
      for(let proxy of this[items$].values()) {
        let value = proxy[readPropertySymbol];
        if (value === undefined || value.size === 0) continue;
        try {
          const ary = [];
          value.forEach(item => ary.push(new Ident(item[id$])));
          yield [collname, '=', new Ident(proxy[id$]), name, ary];
        } catch(e) {
          // ignore revoke errors
        }
      }
