...
const { assertConstantsDefined, getNth } = require('../../util');

const { idSym:         id$
      , collectionSym: collection$
      , itemSym:       item$ } = require('../symbols');

assertConstantsDefined({id$, collection$, item$}, 'symbol');
      ...
      get(map, name) {
        ...
        switch(name) {
          ...
          case 'iter':
            return iter(map);
          case 'forEach':
            return map[forEach$] || (map[forEach$] = (cb, cx) => map.forEach(cb, cx));
          case 'clear':
            return unmodifiable;
          default:
            if ('string' === typeof name && numberRegexp.test(name)) {
              return getNth(map.entries(), name);
            }
            else {
              value = map[name];
