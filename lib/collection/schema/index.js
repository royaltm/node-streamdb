...
const schemaProto = freeze({
  ...
  [$extend](descr, prefix) {
    ...
    if (name.length === 0) throw new SchemaSyntaxError(`schema: property name must not end with a "." in ${descr.name}`);
    ...
    for(let offset = name.indexOf('.'); offset > -1; offset = name.indexOf('.', offset)) {
      if (offset === 0) throw new SchemaSyntaxError(`consecutive "." separator in schema property ${descr.name}`);
...
exports.createSchemaDescriptor = function(property, schemaCfg) {
  if (property.startsWith('__')) throw new SchemaSyntaxError(`collection property name must not start with "__": ${this._name}:${property}`);
  if (property.startsWith('.')) throw new SchemaSyntaxError(`collection property name must not start with ".": ${this._name}:${property}`);
