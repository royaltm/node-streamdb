...
/* exports item as object without many side of references as instances of Ident and without "_id" property
   suitable for update values */
exports.exportSchemaItemNoManyReferences = function exportSchemaItemNoManyReferences(res, schema, item) {
  for(const name in item) {
    const value = item[name]
      , descr = schema[name]
      , collection = descr && descr.collection;
    ...
    if (collection !== undefined) {
      if (value !== undefined && !descr.hasMany) {
        res[name] = new Ident(value[id$]);
      }
    }
    else if (value !== undefined) {
...
/*
  ...
  optional properties:
    ...
    {["*"]: {...}} // wildcard property
*/
exports.toSchemaObject = function toSchemaObject(schema, item, maxlevel, cache, properties=emptyObject) {
  var res = {_id: item[id$]};
  ...
  for(const name in item) {
    ...
    if (collection !== undefined) {
      if (prop === undefined) prop = properties['*'] || emptyObject;
      let sublevel = prop[""];
      if (sublevel === undefined) sublevel = maxlevel;
      ...
      if (descr.hasMany) {
        ...
        if (sublevel < 0) {
          ...
          value.forEach(item => ary.push(cache.has(item) ? item[id$]
                                                         : toSchemaObject(schema, item, sublevel, cache, prop)));
        }
      }
      else if (value !== undefined) {
        res[name] = (sublevel < 0 || cache.has(value)) ? value[id$]
                                                       : toSchemaObject(collection[schema$], value, sublevel, cache, prop);
      }
    }
    else if (value !== undefined) {
