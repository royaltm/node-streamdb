    ...
    set(value) {
      ...
      try {
        ...
        if (value !== undefined && compositeUnique.has(keys)) {
          throw new UniqueConstraintViolationError(`unique constraint violated: ${descr.indexName}(${keys.map(String).join(',')}) ${collectionName}["${this[id$]}"].${descr.indexComponentName} = ${String(value)}`);
        }
