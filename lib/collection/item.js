...
/**
 * converts item and foreign items to plain objects recursively
 *
 ...
 * if maxlevel > 0 foreign items are converted to objects up to maxlevel
 * otherwise they are converted to their id strings
 *
 * to prevent circular references circular foreign items are converted to their id strings
 *
 * @param {Item} item
 ...
 *
 * returns an array of tuples: [collection, id, value]
 *
 * modifies input objects in place
 *
 * example:
 *
 *     Item.unwrap(collection, input).forEach(([coll, id, obj]) => { coll[id] = obj });
 *
...
function unwrap(collection, input) {
  ...
  if (isArray(input)) input.forEach(obj => unwrapSchemaObjects(collection, obj, result));
  else unwrapSchemaObjects(collection, input, result);
