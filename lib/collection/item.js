 ...
 *
 * hiddenProperties may be given as: {[propertyToHide]: false}
 *
 * @param {Item} item
 * @param {number} [maxlevel=0]
 * @param {Object} [hiddenProperties]
 * @return {Object|string}
**/
function toObject(item, maxlevel, hiddenProperties) {
  maxlevel = trunc(maxlevel) || 0;

  if (item instanceof Item) {
    if (maxlevel < 0) return item[id$];
    return toSchemaObject(item[collection$][schema$], item, maxlevel, maxlevel > 0 && new Set(), hiddenProperties);
  }
  else if (item !== null && 'object' === typeof item && 'function' === typeof item[iterator]) {
    const res = [];
    for(const elm of item) {
      res.push(toObject(elm, maxlevel, hiddenProperties));
    }
    return res;
  }
  else {
