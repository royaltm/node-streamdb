...
, flatmap(mapper, maxdepth) {
    ...
    else return new Iterator(flatmap(this, mapper, [], maxdepth));
  }
...
, tail() {
    return new Iterator(drop(this, 1));
  }
...
function* flatmap(iter, mapper, counters, maxdepth) {
  var i = 0, depth = counters.length;
  for(var item of iter) {
    if ('function' === typeof item[iterator$] && !(maxdepth <= depth)) {
      counters[depth] = i;
      yield *flatmap(item[iterator$](), mapper, counters, maxdepth);
      counters.length = depth;
    }
    else yield mapper(item, ...counters, i);
