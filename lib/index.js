...
class DB extends EventEmitter {
  ...
  _push(collection, operator, filter, options, data) {
    ...
    if (spool === undefined) spool = this._spool = [genIdent()];
  ...
  [$update](args) {
    assert(isArray(args) && args.length !== 0, "DB[$update]: argument is not an array or is empty");
    const ident = args[0];
    // console.log('update: ' + ident);
    assert(args.length % 5 === 1, "DB[$update]: argument length is not a multiple of 5");
    var result, error;
