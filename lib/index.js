...
const { assertConstantsDefined
      , isString, isObject, isArray } = util;
...
const createDBStream = require('./stream');
...
const update$      = Symbol.for("update")
    ...
    , skipHasManyReferences$     = Symbol.for('skipHasManyReferences')
    , hasManyReferencesOnly$     = Symbol.for('hasManyReferencesOnly');
...
class DB extends EventEmitter {
  ...
  **/
  ...
  /**
   * clears (rollbacks) pending updates
  **/
  clearPending() {
    this._spool = undefined;
  }
  ...
  **/
  get stream() {
    var stream = createDBStream(this, this.options.stream);
    super.once('writable', () => { stream.isReadableStreaming = true });
  ...
  **/
  ...
  /**
   * Verifies if semver matches with schema version
   *
   * If semver has different major from schema major
   * or semver minor is greater than schema version minor
   * or semver could not be parsed throws a VersionError.
   *
   * @param {string} semver
   * @return {Object} parsed version;
   **/
  verifyDataVersion(semver) {
    const readVersion = version.parse(semver);
    if (!readVersion) throw new VersionError(`Could not parse version read from stream: "${String(semver)}"`);
   ...
   *
   * semver if provided is verified against current schemaVersion
   * may throw in this instance
   *
   * @param {string} [semver] optional data schema version
   * @return {Object} parsed schema version;
   **/
  pushVersionMark(semver) {
    const schemaVersion = semver === undefined ? this.schemaVersion
                                               : this.verifyDataVersion(semver);
    this._push('_version', INTERNAL_OPERATOR, schemaVersion.version, null, null);
    return schemaVersion;
  }

  /**
   * Pushes error to update log
   *
...
DB.prototype.createDataExporter = function* () {
  ...
  for(var name in collections) {
    yield *collections[name][this$][export$](skipHasManyReferences$);
  }
  for(var name in collections) {
    yield *collections[name][this$][export$](hasManyReferencesOnly$);
  }
};
...
function flush() {
  ...
  if (data !== undefined) {
    ...
    return this.stream.produce(data);
  }
}
...
function onReadVersion(ver) {
  var readVersion;
  try {
    readVersion = this.verifyDataVersion(ver);
  } catch(err) {
    this.emit('error', err);
    return;
  }
...
DB.schemaUtils = require('./collection/schema/utils');
