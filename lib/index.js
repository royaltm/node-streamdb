...
class DB extends EventEmitter {
  ...
  **/
  constructor(options) {
    ...
    if (!schemaVersion) throw new SchemaSyntaxError(`Invalid schema version: ${schemaVersion.toString()}`);
...
function onReadVersion(ver) {
  ...
  try {
    if (!readVersion) throw new VersionError(`could not read version: "${String(ver)}"`);

    const schemaVersion = this.schemaVersion;
    if (schemaVersion.major !== readVersion.major) {
      throw new VersionError(`version read has different major: ${readVersion.major} <> ${schemaVersion.major}`);
    }
    if (schemaVersion.minor < readVersion.minor) {
      throw new VersionError(`version read has greater minor: ${readVersion.minor} > ${schemaVersion.minor}`);
    }
  } catch(err) {
    this.emit('error', err);
    return;
