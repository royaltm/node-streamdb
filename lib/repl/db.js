 ...
 * - `.export`   - Export database to specified file, encoded with msgpack and gzipped
 * - `.import`   - Import database from specified file, unzipped and decoded with msgpack
 * 
...
exports.databaseRepl = function(repl, options) {
  ...
  if (options.defineActions) {
    ...
    repl.defineCommand('export', {
      help: 'Export database to a specified file',
      action: (filename) => {
        if (!filename) {
          console.log(yellow("Please specify filename to save exported database!"));
          prompt(repl);
        }
          ...
          new Promise((resolve, reject) => {
            ...
            new IteratorReader(exportIterator).on('error', reject)
            .pipe(createEncodeStream({codec: codec})).on('error', reject)
            .pipe(createGzip({level: Z_BEST_COMPRESSION})).on('error', reject)
            .pipe(fileWriter).on('error', reject)
            .on('finish', resolve);
          })
          .then(() => console.log('Database exported successfully to: %s', filename))
          .catch(err => {
    ...
    repl.defineCommand('import', {
      help: 'Import database from a specified file',
      action: (filename) => {
        if (!filename) {
          console.log(yellow("Please specify filename to load imported database!"));
          prompt(repl);
        }
        else if (repl.context[dbprop]) {
          ...
          new Promise((resolve, reject) => {
            ...
            .pipe(createUnzip()).on('error', reject)
            .pipe(createDecodeStream({codec: codec})).on('error', reject)
            .pipe(new ImportTransform()).on('error', reject)
            .on('end', resolve)
            .pipe(db.stream, {end: false}).on('error', reject);
          })
          .then(() => console.log('Database imported successfully from: %s', filename))
          .catch(err => {
