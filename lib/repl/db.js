    ...
    defineCommand('import', {
      ...
      action: (pathname) => {
        if (!pathname) {
          ...
          let start = Date.now();
          importDbFromYaml(repl.context[dbprop], pathname, false, console.log)
    ...
    defineCommand('dump', {
      ...
      action: (filename) => {
        if (!filename) {
          ...
          let start = Date.now();
          dumpDatabase(repl.context[dbprop], filename, {flags: 'wx'})
    ...
    defineCommand('slurp', {
      ...
      action: (filename) => {
        if (!filename) {
          ...
          let start = Date.now();
          restoreLocalDatabase(repl.context[dbprop], filename)
    ...
    defineCommand('restore', {
      ...
      action: (filename) => {
        if (!filename) {
          ...
          let start = Date.now();
          restoreDatabase(repl.context[dbprop], filename)
