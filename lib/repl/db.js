...
const codec = require('../../extras/msgpack_lite_codec');
  ...
  if (options.defineActions) {
    ...
    defineCommand('export', {
      ...
      action: (pathname) => {
        ...
        else if (repl.context[dbprop]) {
          let start = Date.now();
          exportDbToYamls(repl.context[dbprop], pathname, console.log)
          .then(() => console.log('Database exported successfully to: %s in %d s', pathname, ((Date.now() - start)/1000).toFixed(3)))
          .catch(err => {
    ...
    defineCommand('import', {
      ...
      action: (pathname) => {
        ...
        if (!pathname) {
          ...
          let start = Date.now()
          importDbFromYaml(repl.context[dbprop], pathname, !!opts.mergeImport, console.log, opts.updateChunkSize)
          .then(() => console.log('Database imported successfully from: %s in %d s', pathname, ((Date.now() - start)/1000).toFixed(3)))
          .catch(err => {
    ...
    defineCommand('dump', {
      ...
      action: (filename) => {
        ...
        if (!filename) {
          ...
          const opts = getOpts(repl);
          let start = Date.now();
          dumpDatabase(repl.context[dbprop], filename, {flags: 'wx', compressionLevel: opts.compressionLevel})
          .then(() => console.log('Database dumped to: %s in %d s', filename, ((Date.now() - start)/1000).toFixed(3)))
          .catch(err => {
    ...
    defineCommand('slurp', {
      ...
      action: (filename) => {
        ...
        if (!filename) {
          ...
          const opts = getOpts(repl);
          let start = Date.now();
          restoreLocalDatabase(repl.context[dbprop], filename, !!opts.noUnzip)
          .then(() => console.log('Database slurped successfully from: %s in %d s', filename, ((Date.now() - start)/1000).toFixed(3)))
          .catch(err => {
    ...
    defineCommand('restore', {
      ...
      action: (filename) => {
        ...
        if (!filename) {
          ...
          const opts = getOpts(repl);
          let start = Date.now();
          restoreDatabase(repl.context[dbprop], filename, !!opts.noUnzip, opts.updateChunkSize)
          .then(() => console.log('Database restored successfully from: %s in %d s', filename, ((Date.now() - start)/1000).toFixed(3)))
          .catch(err => {
